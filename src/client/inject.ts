...
const tweetSelector = '[data-testid="tweet"]:not(:has(.warpcast-button))';
...
const quoteToWarpcast = (tweetAddress) => {
    const composeWarcastUrl = `https://warpcast.com/~/compose?text=`
    const text = encodeURIComponent(` ${tweetAddress}`);
    window.open(`${composeWarcastUrl}${text}`, '_blank');
}

const insertQuoteToWarpcastButon = (tweetElement) => {
    const button = tweetElement.querySelector('.warpcast-button');
    if (button) {
        return;
    }
    ...
    const warpcastButton = document.createElement('button');
    warpcastButton.className = 'warpcast-button';
    warpcastButton.innerHTML = SVG;
    warpcastButton.style = 'background: transparent;border: 1px solid #3c2173;padding: 0.2rem; padding-left: 0.5rem; padding-right: 0.5rem; border-radius: 0.5rem; color: #3c2173; font-size: 0.8rem; font-weight: 600; display: flex; align-items: center; justify-content: center; cursor: pointer;';
    warpcastButton.onclick = () => {
        quoteToWarpcast(tweetAddress);
    }
    ...
    bookmarkButtonParent.prepend(warpcastButton);
}
...
const callback = function(mutationsList, observer) {
    for(const mutation of mutationsList) {
            ...
            tweets.forEach(tweet => {
                insertQuoteToWarpcastButon(tweet);
            });
    }
}
